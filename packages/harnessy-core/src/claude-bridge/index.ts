...
import { extractAgentsAppend } from "./agents-md.ts";
import { buildActionSummary, type ToolCallState } from "./askclaude-ui.ts";
import { type Config, loadConfig } from "./config.ts";
import { convertPiMessages, messageContentToText, PROVIDER_ID } from "./convert.ts";
...
import { ctx, QueryContext } from "./query-state.ts";
import { verifyWrittenSession as _verifyWrittenSession } from "./session-verify.ts";
import { extractSkillsBlock, MCP_SERVER_NAME, MCP_TOOL_PREFIX } from "./skills.ts";
import { jsonSchemaToZodShape } from "./typebox-to-zod.ts";
...
// --- Debug logging ---
// HARNESSY_CLAUDE_BRIDGE_DEBUG=1 enables logging in the active Harnessy agent directory.

const DEBUG = process.env.HARNESSY_CLAUDE_BRIDGE_DEBUG === "1";

function debugLogPath(): string {
	return process.env.HARNESSY_CLAUDE_BRIDGE_DEBUG_PATH || join(getHarnessyAgentDir(), "claude-bridge.log");
}

function diagLogPath(): string {
	return join(getHarnessyAgentDir(), "claude-bridge-diag.log");
}
...
function debug(...args: unknown[]) {
	...
	appendFileSync(path, `[${ts}] [${moduleInstanceId}] ${msg}\n`);
}

// Per-query CLI debug capture. When HARNESSY_CLAUDE_BRIDGE_DEBUG=1, ask the Claude Code
...
const ACTIVE_STREAM_SIMPLE_KEY = Symbol.for("harnessy:claude-bridge:activeStreamSimple");
	...
	for (const msg of warnings) {
		...
		piUI?.notify(
			`Session file issue: ${msg}\n` +
				...
				(DEBUG
					? ` and attach ${debugLogPath()}`
					: ` (rerun with HARNESSY_CLAUDE_BRIDGE_DEBUG=1 to capture a debug log)`),
			"warning",
		);
...
function processAssistantMessage(
	...
	for (const block of assistantMsg.content) {
		if (block.type === "text" && block.text) {
			...
			c.currentPiStream?.push({ type: "text_start", contentIndex: idx, partial: turnOutput });
			c.currentPiStream?.push({ type: "text_delta", contentIndex: idx, delta: block.text, partial: turnOutput });
			c.currentPiStream?.push({ type: "text_end", contentIndex: idx, content: block.text, partial: turnOutput });
		} else if (block.type === "thinking") {
			ensureTurnStarted(c);
			c.turnBlocks.push({
				type: "thinking",
			...
			const idx = c.turnBlocks.length - 1;
			c.currentPiStream?.push({ type: "thinking_start", contentIndex: idx, partial: turnOutput });
			if (block.thinking)
				c.currentPiStream?.push({
					type: "thinking_delta",
					contentIndex: idx,
			...
			c.currentPiStream?.push({ type: "toolcall_start", contentIndex: idx, partial: turnOutput });
			c.currentPiStream?.push({
				type: "toolcall_end",
				contentIndex: idx,
	...
	if (assistantMsg.usage) updateUsage(turnOutput, assistantMsg.usage, model);
...
async function promptAndWait(
	...
	// Skills append
	const skillsBlock =
		options?.appendSkills !== false && options?.systemPrompt ? extractSkillsBlock(options.systemPrompt) : undefined;

	// Effort
	const effort = options?.thinking && options.thinking !== "off" ? REASONING_TO_EFFORT[options.thinking] : undefined;
...
export function harnessyClaudeBridgeExtension(pi: ExtensionAPI) {
	// Disable non-essential Claude Code traffic (update checks, MCP registry, telemetry)
	process.env.CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC = "1";

	const config = loadConfig(process.cwd());
	debug("loadConfig:", JSON.stringify(config));
	const executableOverride = process.env.HARNESSY_CLAUDE_CODE_BIN?.trim();
	providerSettings = {
		...config.provider,
		...(executableOverride ? { pathToClaudeCodeExecutable: executableOverride } : {}),
	};

	pi.registerCommand("claude-auth", {
		description: "Show Claude Code authentication status",
		handler: async (_args, commandContext) => {
			const executable = providerSettings.pathToClaudeCodeExecutable ?? "claude";
			try {
				const result = await pi.exec(executable, ["auth", "status", "--text"], { timeout: 15_000 });
				if (result.code === 0) {
					commandContext.ui.notify(result.stdout.trim() || "Claude Code is authenticated.", "info");
					return;
				}
				const detail = result.stderr.trim() || result.stdout.trim() || `exit code ${result.code}`;
				commandContext.ui.notify(
					`Claude Code authentication is unavailable (${detail}). Run \`${executable} auth login\` in your terminal.`,
					"warning",
				);
			} catch (error) {
				commandContext.ui.notify(
					`Claude Code could not be started (${errorMessage(error)}). Install it, then run \`${executable} auth login\`.`,
					"warning",
				);
			}
		},
	...
	if (askConf?.enabled === true) {
		const askClaudeParams = Type.Object({
			...
			model: Type.Optional(
				Type.String({
					description: 'Claude model (e.g. "opus", "sonnet", "haiku", or full ID). Defaults to "opus".',
				}),
			),
		...
		pi.registerTool<typeof askClaudeParams>({
			...
			renderCall(args, theme) {
				...
				const truncated =
					args.prompt.length > PREVIEW_MAX_CHARS ? args.prompt.substring(0, PREVIEW_MAX_CHARS) : args.prompt;
			...
			renderResult(result, { expanded, isPartial }, theme) {
				...
				const details = result.details as
					| { prompt?: string; executionTime?: number; actions?: string; error?: boolean }
					| undefined;
				const body = result.content[0]?.type === "text" ? result.content[0].text : "";

				let text = details?.error ? theme.fg("error", "✗ Claude Code error") : theme.fg("mdLink", "✓ Claude Code");
