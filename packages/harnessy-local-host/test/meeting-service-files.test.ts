...
it.runIf(process.platform === "darwin")(
	"disable binds the loaded path and submits stop after blocking relaunch",
	() => {
		const root = fixture();
...
it.runIf(process.platform === "darwin")(
	"disable rejects another installation and propagates failed stop without retries",
	() => {
		const root = fixture();
		const plist = renderMeetingServiceLaunchAgent("/fixture/node", "/fixture/cli.js", "/fixture/service.json", root);
		installMeetingServiceFiles(root, plist);
...
it.runIf(process.platform === "darwin")("disable an absent service without pretending to stop a process", () => {
	const root = fixture();
	const plist = renderMeetingServiceLaunchAgent("/fixture/node", "/fixture/cli.js", "/fixture/service.json", root);
	installMeetingServiceFiles(root, plist);
	const calls: string[] = [];
...
it.runIf(process.platform === "darwin")(
	"waits for asynchronous removal but rejects unknown state and a stuck service",
	async () => {
		vi.useFakeTimers();
		let checks = 0;
...
it.runIf(process.platform === "darwin")(
	"enable validates exact files before submitting bounded service control",
	() => {
		const root = fixture();
		const plist = renderMeetingServiceLaunchAgent("/fixture/node", "/fixture/cli.js", "/fixture/service.json", root);
		installMeetingServiceFiles(root, plist);
		...
		calls.length = 0;
		expect(() => controlMeetingServiceFiles(root, `${plist}changed`, boundary)).toThrow(
...
it.runIf(process.platform === "darwin")(
	"enable refuses existing or unknown owners and never retries a failed submission",
	() => {
		const root = fixture();
		const plist = renderMeetingServiceLaunchAgent("/fixture/node", "/fixture/cli.js", "/fixture/service.json", root);
		installMeetingServiceFiles(root, plist);
		for (const failure of ["print-loaded", "print-unknown", "enable", "bootstrap"]) {
			const calls: string[] = [];
			expect(() =>
				controlMeetingServiceFiles(root, plist, (args) => {
					const action = args[0]!;
					calls.push(action);
					if (action === "print")
...
it.runIf(process.platform === "darwin")(
	"stages a valid private launch agent and logs without activating or overwriting",
	() => {
		const root = fixture();
		const plist = renderMeetingServiceLaunchAgent("/fixture/node", "/fixture/cli.js", "/fixture/service.json", root);
