...
import { runMeetingWorkerCommand } from "./meeting-worker-command.ts";
...
try {
	const exit = await Effect.runPromiseExit(runMeetingWorkerCommand(process.argv.slice(2)), {
	...
	if (Exit.isSuccess(exit) && signalExitCode === undefined) {
		...
		process.stderr.write(
			`${JSON.stringify({ error: "meeting_worker_failed", code: signalExitCode === undefined ? "runtime_failed" : "interrupted" })}\n`,
		);
