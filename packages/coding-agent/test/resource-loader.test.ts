...
function withoutBuiltinPlanMode<T extends { path: string }>(extensions: T[]): T[] {
	return extensions.filter((extension) => extension.path !== "<inline:plan-mode>");
}
...
Project skill`,
	...
	pi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			const discoveredExtensions = withoutBuiltinPlanMode(extensionsResult.extensions);
...
export default function(pi) {
		...
		handler: async () => {},
			...
			await loader.reload({
				resolveProjectTrust: async ({ extensionsResult }) => {
					expect(withoutBuiltinPlanMode(extensionsResult.extensions).map((extension) => extension.path)).toEqual([
						join(userExtDir, "user.ts"),
					]);
					return true;
				},
	...
	pi.registerCommand("user-only", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(withoutBuiltinPlanMode(extensionsResult.extensions)).toHaveLength(2);
			...
			const commands = runner.getRegisteredCommands().filter((command) => command.invocationName !== "plan");
...
Project skill content`,
			...
			expect(withoutBuiltinPlanMode(loader.getExtensions().extensions)).toHaveLength(0);
