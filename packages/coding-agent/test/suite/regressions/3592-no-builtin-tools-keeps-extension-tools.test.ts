...
describe("regression #3592: no-builtin-tools keeps extension tools enabled", () => {
	...
	it("keeps extension tools active when built-in defaults are disabled", async () => {
		...
		expect(
			session
				...
				.map((tool) => tool.name)
				.sort(),
		).toEqual(["bash", "dynamic_tool", "edit", "enter_plan_mode", "find", "grep", "ls", "read", "write"]);
		expect(session.getActiveToolNames()).toEqual(["enter_plan_mode", "dynamic_tool"]);
	...
	it("propagates noTools through service-based session creation", async () => {
		...
		expect(session.getActiveToolNames()).toEqual(["enter_plan_mode"]);
		expect(session.systemPrompt).toContain(
			"- enter_plan_mode: Enter read-only plan mode before implementation when planning would help",
		);
		expect(session.systemPrompt).not.toContain("- read:");
		session.dispose();
	});
